...
class FeatureMatrixDetails
{
    ...
    internal Func<IRegexEngine, FeatureMatrix, bool>? DirectCheck { get; init; } // (in continuation of rules)
    ...
    internal static readonly FeatureMatrixGroup[] AllFeatureMatrixDetails =
        [
            ...
            [
                ...
                new ( @"[:class:]", @"Character class outside sets", fm => fm.Class_Name, @"[:alpha:]", "X", null ),
            ] ),
            ...
            [
                ...
                new ( @"[[.elem.]]", @"Collating symbol", fm => fm.InsideSets_Collating, @"a[[.ch.]]x", "achx", null, @"a[[.comma.]]b", "a,b", null ), // STL seems to have a defect.
            ] ),
            ...
            new ( @"Grouping",
            [
                new ( @"(?:…)", @"Non-capturing group", fm => fm.NoncapturingGroup, @"(?:x)", "x", null, @"\(?:x\)", "x", null ),
        ...
        new ( @"Miscellaneous",
            [
                ...
                new ( @"Unicode", @"Supports Unicode characters, not just ASCII", fm => !fm.AsciiOnly, @"X.....Y", "XăîșțâY", null ),
                new ( @"Surrogates", @"“.” matches surrogate pairs as one entity (no split)", fm => !fm.AsciiOnly && !fm.SplitSurrogatePairs, @"X.Y", "X💕Y", null ),

                new ( @"Fuzzy matching", @"Approximate matching using special patterns or parameters", fm => fm.Quantifier_Braces_FreeForm == FeatureMatrix.PunctuationEnum.Normal || fm.Quantifier_Braces_FreeForm == FeatureMatrix.PunctuationEnum.Backslashed || fm.FuzzyMatchingParams,
                        @"(test){i}", "teXst", null,
                        @"(test){+1}", "teXst", null,
                        @"\(test\)\{+1\}", "teXst", null
                        ) { DirectCheck = (e, fm) => fm.FuzzyMatchingParams },
            new ( "No hang", "No catastrophic infinite matching, no timeout errors", fm => fm.TreatmentOfCatastrophicPatterns == FeatureMatrix.CatastrophicBacktrackingEnum.Accept ) { DirectCheck = (e, fm) => CheckCatastrophicPattern( e, fm ) == CatastrophicBacktrackingResultEnum.Passed },
