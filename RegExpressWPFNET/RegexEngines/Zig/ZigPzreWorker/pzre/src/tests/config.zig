...
test "pzre ignore_case" {
  ...
  try testMatchManyWithConfig("HeLlO", &.{
    .{ .str = "hello", .expected = Match{ .str = "hello", .loc = .init(0, 5) } },
    .{ .str = "HELLO", .expected = Match{ .str = "HELLO", .loc = .init(0, 5) } },
  }, config);
  ...
  // 2. Character Sets
  try testMatchManyWithConfig("[ab]", &.{
    .{ .str = "A", .expected = Match{ .str = "A", .loc = .init(0, 1) } },
  ...
  try testMatchManyWithConfig("[@-[]", &.{
    .{ .str = "@", .expected = Match{ .str = "@", .loc = .init(0, 1) } },
    .{ .str = "`", .expected = null },
  }, config);
  
  try testMatchManyWithConfig("[`-z]", &.{
    .{ .str = "`", .expected = Match{ .str = "`", .loc = .init(0, 1) } },
    .{ .str = "@", .expected = null },
  }, config);
...
test "pzre pat_ignore_whitespace semantics" {
  ...
  try testMatchManyWithConfig("a b", &.{
    .{ .str = "a b", .expected = Match{ .str = "a b", .loc = .init(0, 3) } },
...
test "pzre custom sets injection" {
  ...
  try testMatchManyWithConfig("\\d", &.{
    .{ .str = "5", .expected = Match{ .str = "5", .loc = .init(0, 1) } },
    .{ .str = "9", .expected = null },
  }, config);

  // \w should match b, but strictly fail on z
  try testMatchManyWithConfig("\\w", &.{
    .{ .str = "b", .expected = Match{ .str = "b", .loc = .init(0, 1) } },
    .{ .str = "z", .expected = null },
  }, config);
