...
pub const List = enum {
  ...
  pub fn Fn(list: List, comptime T: type, comptime size: ?usize) type {
    return switch (list) {
      ...
      .comptime_linked_list => ComptimeLinkedList(T),
  ...
  pub const single_ended = struct {
    pub fn listMapping(comptime memory_model: MemoryModel) List {
      return switch (memory_model) {
        .dynamic => .array_list,
        // .fixed => .bounded_array,
        .comptime_dynamic => .comptime_linked_list,
      };
    }
  ...
  return struct {
    ...
    pub inline fn append(self: *Self, gpa: Allocator, item: Element) Error!void {
      switch (comptime list) {
        ...
        .comptime_linked_list => self.data.append(item),
    ...
    pub inline fn dupeInplace(self: *Self, gpa: Allocator) Error!void {
      switch (comptime list) {
        ...
        .comptime_linked_list => self.data.dupeInplace(),
    ...
    pub inline fn pushFront(self: *Self, gpa: Allocator, item: Element) Error!void {
      switch (comptime list) {
        ...
        .comptime_linked_list => self.data.pushFront(item),
    ...
    pub inline fn appendSlice(self: *Self, gpa: Allocator, items: []const Element) Error!void {
      switch (comptime list) {
        ...
        .comptime_linked_list => self.data.appendSlice(items),
    ...
    pub inline fn initUsing(items: []const Element) Self {
      const data = switch (comptime list) {
        ...
        .comptime_linked_list => ComptimeLinkedList(Element).initUsing(items),
        .array_list => ArrayList(Element){.items = @constCast(items), .capacity = items.len},
        else => Deque(Element).initUsing(items),
      };
    ...
    pub inline fn len(self: *const Self) usize {
      return switch (comptime list) {
        ...
        .comptime_linked_list => self.data.len,
    ...
    pub inline fn deinit(self: *Self, gpa: Allocator) void {
      switch (comptime list) {
        .comptime_array_list, .comptime_linked_list, .bounded_array => {},
        .array_list, .deque => self.data.deinit(gpa),
      }
    }
    ...
    pub inline fn freeOwnedSlice(self: *Self, gpa: Allocator, slice: []const Element) void {
      _ = self;
      switch (comptime list) {
        .comptime_array_list, .comptime_linked_list => {},
        else => gpa.free(slice),
      }
    }
    ...
    /// 
    /// Only mutable if comptime_linked_list, due it requiring to slice itself and then cache the result
    pub inline fn get(self: *Self, index: usize) Element {
      return switch (comptime list) {
        .comptime_array_list => self.data.items[index],
        .comptime_linked_list => self.data.getItems()[index],
    ...
    /// It has to be mutable due to the comptime linked list that caches this expensive action
    pub inline fn getConstSlice(self: *Self) []const Element {
      return switch (comptime list) {
        .comptime_array_list => self.data.items,
        .comptime_linked_list => self.data.getItems(),
    ...
    pub inline fn getSlice(self: *Self) []Element {
      return switch (comptime list) {
        .comptime_array_list, .comptime_linked_list => @compileError("Not possible"),
        .bounded_array => self.data.buffer[0..self.data.len],
        .array_list => self.data.items,
        // Assuming Deque has random access via get()
        .deque => @compileError("Not possible"),
    ...
    pub inline fn getPtr(self: *Self, index: usize) *Element {
      return switch (comptime list) {
        .comptime_array_list, .comptime_linked_list => @compileError("Unimplemented"),
    ...
    pub inline fn set(self: *Self, index: usize, item: Element) void {
      switch (comptime list) {
        ...
        .comptime_linked_list => self.data.set(index, item),
    ...
    /// 'other' is mutable only if its a comptime_linked_list
    pub inline fn appendOther(self: *Self, gpa: Allocator, other: *Self) Error!void {
      switch (comptime list) {
        .comptime_array_list => self.data.appendSlice(other.data.items),
        .comptime_linked_list => self.data.appendSlice(other.data.getItems()),
    ...
    pub inline fn clearRetainingCapacity(self: *Self) void {
      switch (comptime list) {
        ...
        .comptime_linked_list => self.data.clearRetainingCapacity(),
    ...
    pub inline fn ensureUnusedCapacity(self: *Self, gpa: Allocator, amount: usize) Error!void {
      switch (comptime list) {
        .comptime_array_list, .comptime_linked_list, .bounded_array => {},
        .array_list => try self.data.ensureUnusedCapacity(gpa, amount),
        .deque => try self.data.ensureUnusedCapacity(gpa, amount),
      }
    }

    pub inline fn ensureTotalCapacityPrecise(self: *Self, gpa: Allocator, n: usize) Error!void {
      return switch (comptime list) {
        .comptime_array_list, .comptime_linked_list, .bounded_array => {},
        .array_list, .deque => try self.data.ensureTotalCapacityPrecise(gpa, n),
      };
    }
    ...
    pub inline fn clone(self: *const Self, gpa: Allocator) Error!Self {
      ...
      switch (comptime list) {
        ...
        .comptime_linked_list => new.data = self.data.clone(),
...
fn testPolyListOperations(list: anytype, gpa: Allocator) !void {
  ...
  if (comptime @TypeOf(list.*).list != .deque) {
    ...
    if (comptime @TypeOf(list.*).list != .comptime_array_list and @TypeOf(list.*).list != .comptime_linked_list) {
...
test "PolymorphicList: Verify all backends" {
  ...
  // 4. Comptime Dynamic (Linked List now active via presets)
  comptime {
  ...
  switch (mem_model) {
    ...
    .comptime_dynamic => {
      comptime {
        ...
        const frozen_ranges = new.ranges ++ &[0]Range{};
        self.data = .initUsing(frozen_ranges);
      }
    }
  }
}
