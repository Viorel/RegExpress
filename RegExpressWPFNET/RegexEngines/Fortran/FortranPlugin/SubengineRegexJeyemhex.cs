...
partial class SubengineRegexJeyemhex( Options options ) : RegexSubengine
{
    ...
    {
        Debug.Assert( options.Module == ModuleEnum.RegexJeyemhex );
...

            {
                ...
                if( m.Success )
                {
                    ...
                    if( native_start > 0 )
                    {
                        ...
                        if( native_length < 0 ) native_length = 0; // empty match
    ...
    static string GetWorkerExePath( )
    {
        ...
        string worker_exe = Path.Combine( assembly_dir, @"FortranRegexJeyemhexWorker.bin" );
