...
#: The name CPython 3.13 gave :class:`error`, kept as an alias there and the spelling any
#: ``except re.PatternError`` written since is looking for. The same class, not a subclass,
#: so either spelling catches what the other raises.
PatternError = error
...
def _compile_repr(pattern, flags):
    """``real.compile(..., fallback=True)`` form for a delegated pattern (%.200R, named flags).

    The ``fallback=True`` is not decoration: this repr describes a pattern running on the
    backtracking ``re``, and without it the string rebuilds a *different* object — or raises,
    since the same call without the argument is what the engine refused. It is also the one
    place a reader sees, at a glance in a log or a debugger, that this pattern forfeits the
    linear-time guarantee. :meth:`_FallbackPattern.__reduce__` has always passed it.
    """
    shown = flags & ~UNICODE
    names = []
    for bit, name in (
        ...
        if shown & bit:
            # "real.", not "re.": see the matching note in _real.cpp's pattern_flags_suffix.
            names.append("real." + name)
    body = repr(pattern)
    if len(body) > 200:
        body = body[:200]
    if names:
        return "real.compile(" + body + ", " + "|".join(names) + ", fallback=True)"
    return "real.compile(" + body + ", fallback=True)"
...
class RegexSet:
    ...
    def __init__(self, patterns, flags=0):
        ...
        """
        # Same \N{NAME} rewrite compile() does: the engine only ever understands \N{U+XXXX}, and a
        # name is resolved here. Without this a set member spelled \N{LATIN SMALL LETTER A} was
        # refused by the message that tells you the binding resolves it — which this one did not.
        patterns = [_rewrite_named_chars(p, flags) if isinstance(p, str) else p for p in patterns]
...
def _verbose_in_effect(pattern, flags):
    """Whether an unescaped ``#`` starts a comment, for the rewrite below only.

    Two sources, both decidable without parsing: the ``VERBOSE`` bit, and a *global* inline
    ``(?x)``. The second is cheap because since Python 3.11 a global inline flag group must be the
    pattern's first construct — `re` and REAL both reject `a(?x)b` with "global flags not at the
    start" — so one anchored match settles it. A **scoped** ``(?x:…)`` is deliberately not tracked:
    see the note on the residue in :func:`_rewrite_named_chars`.
    """
    if flags & VERBOSE:
        return True
    head = _re.match(r"\(\?([aimsuxL]+)\)", pattern)
    return bool(head and "x" in head.group(1))


def _rewrite_named_chars(pattern, flags=0):
    ...
    Comment text is skipped -- ``(?#...)`` anywhere, and ``#`` to end-of-line when VERBOSE is in
    effect -- because `re` never parses a comment's contents, so a bogus name there must not raise
    here either. Neither form is a comment inside a character class, which is tracked for that
    reason alone.

    **Residue, named rather than approximated:** a *scoped* ``(?x:...)`` also enables VERBOSE, and
    following it means tracking a flag-scope stack -- push/pop, ``(?-x:...)``, ``(?x-i:...)`` --
    which the C++ parser already implements. Reproducing it here would be a second model of the
    language to keep true, so ``\N{BOGUS}`` inside a scoped-verbose comment still raises where `re`
    is silent. One shape, decidable only by parsing, left to the parser.
    """
    if "\\N{" not in pattern:
    ...
    i = 0
    n = len(pattern)
    verbose = _verbose_in_effect(pattern, flags)
    in_class = False
    while i < n:
        # A comment's text is not pattern syntax: `re` never parses `\N{…}` there, so neither may
        # this rewrite. Two forms, and neither exists inside a character class.
        if not in_class and pattern.startswith("(?#", i):
            ...
            i = close + 1
            continue
        if not in_class and verbose and pattern[i] == "#":
            end = pattern.find("\n", i)
            ...
            continue
        if pattern[i] != "\\":
            # `]` is a literal right after `[` or `[^`, so the class cannot close on its first char.
            if not in_class and pattern[i] == "[":
                in_class = True
                skip = i + (2 if pattern.startswith("[^", i) else 1)
                if pattern.startswith("]", skip):
                    out.append(pattern[i:skip + 1])
                    i = skip + 1
                    continue
            elif in_class and pattern[i] == "]":
                in_class = False
...
def _compile_cached(pattern, flags):
    ...
    if isinstance(pattern, str):
        pattern = _rewrite_named_chars(pattern, flags)
