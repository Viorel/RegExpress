...
{
	const auto size_needed = MultiByteToWideChar( CP_UTF8, MB_ERR_INVALID_CHARS, s, -1, nullptr, 0 ); // including '\0'
	...
	if( MultiByteToWideChar( CP_UTF8, MB_ERR_INVALID_CHARS, s, -1, &result[0], size_needed ) <= 0 ) // result includes '\0'
	{
...
std::wstring Utf8ToWString( const char* s, int len ) // (see also 'UTF8_to_wchar' from 'PartialJSON.h')
{
	if( len < 0 ) throw std::invalid_argument( "Invalid negative length." );

	const auto size_needed = MultiByteToWideChar( CP_UTF8, MB_ERR_INVALID_CHARS, s, len, nullptr, 0 ); // does not include '\0'
	...
	if( MultiByteToWideChar( CP_UTF8, MB_ERR_INVALID_CHARS, s, len, &result[0], size_needed ) <= 0 ) // result does not include '\0'
	{
		throw std::runtime_error( "MultiByteToWideChar() failed [2]." );
	}

	result.resize( size_needed ); // should add '\0'
...
{
	return std::wstring( (unsigned char*)s.c_str( ), (unsigned char*)( s.c_str( ) + s.length( ) ) );
}
